...
@Deprecated
public abstract class ServuxStructuresHandlerOld<T extends CustomPayload> implements IPluginServerPlayHandler<T>
{
    private static final ServuxStructuresHandlerOld<ServuxStructuresPayloadOld> INSTANCE = new ServuxStructuresHandlerOld<>() {
        @Override
        public void receive(ServuxStructuresPayloadOld payload, ServerPlayNetworking.Context context)
        {
    ...
    public void decodeNbtCompound(Identifier channel, ServerPlayerEntity player, NbtCompound data)
    {
        ...
        switch (data.getInt("packetType"))
        {
            ...
            default -> Servux.logger.warn("ServuxStructuresHandlerOld#decodeC2SNbtCompound(): Invalid packetType from player: {}, of size in bytes: {}.", player.getName(), data.getSizeInBytes());
        }
    }
    ...
    public void receivePlayPayload(T payload, ServerPlayNetworking.Context ctx)
    {
        ...
        {
            ServuxStructuresHandlerOld.INSTANCE.decodeNbtCompound(CHANNEL_ID, ctx.player(), ((ServuxStructuresPayloadOld) payload).data());
        }
    }
    ...
    {
        if (ServuxStructuresHandlerOld.INSTANCE.sendPlayPayload(player, new ServuxStructuresPayloadOld(data)) == false)
        {
